...
/** Base URL for Google Calendar OAuth redirect — uses NEXT_PUBLIC_APP_URL as-is so local dev (localhost) works. */
export function getCalendarRedirectBaseUrl(): string {
  const url = process.env.NEXT_PUBLIC_APP_URL?.trim()
  if (!url) return 'https://brooklinen-backstage.vercel.app'
  return url.replace(/\/$/, '')
}
